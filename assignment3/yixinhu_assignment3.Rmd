...
q2_regression_ready <- q2_regession_df %>% 
  ...
  mutate(group_mean_correct_in_category = mean(correct),
         id_mean_correct_in_category = id_mean_correct_in_category*100,
         group_mean_correct_in_category = group_mean_correct_in_category*100)

test_lm <- glm(data=q2_regression_ready, correct ~ (id_mean_correct_in_category) + (group_mean_correct_in_category) ,family="binomial")
...
predicted_possibilities_q2 <- predict(test_lm, q2_test_ready, type="response")

verdicts_q2 <- rep(0, dim(q2_test_ready)[1])
verdicts_q2[predicted_possibilities_q2 > .5] = 1

test_results <- as.data.frame(cbind(verdicts_q2,q2_test_ready$correct))
...
regression_coefficient_percents <- (exp(coef(test_lm))-1)*100

regression_coefficient_percents
```
In this basic logit regression, where we considered individual's performance as well as how difficult is this category among their peers. Although this extremely parsimonious prediction does give a fairly decent predicted accuracy of around 68%, this the model itself warrants a lot of explanation. 

Stepping back from this the model for a quick second, I would like to restate the idea that we are primarily focused on the first coefficient, and the second variate is more of a means of controlling for the general difficulty of the type of question.

Recall that I have multiplied all the percentages (fractions between 0 and 1) into percent units (numbers between 0 and 100). This is so that our interpretation of the coefficient for the logit regression actually makes sense. First of all, the coefficients are still in log odds ratios, an incredibly cryptic unit. Converting it back to normal percentiles, we get that that coefficient which states that for a percent increase one's own mean correctness in the specified category, there is a 4.767 percent increase in the chances that individual A will get the specific question X right. On the other hand, for the coefficient on the group mean of the particular category, a percent increase in how well your group is doing will indicate a 0.080 percent increase in the individual A's probability of getting the specific question X right. 

To conclude, we have built a model where, by controlling the difficulty of the category in question, we estimate how one's previous performance in the specific question type will influence the rate at which the same individual will get a question right in the same category. Once we have completed the model, we have checked to see that our model has an correct rate of 67.88% by training on the 2017 data and testing on the 2018 data.

NOTES:

Please note that this model is extremely parsimonious, where I left out any analysis on the question level (i.e. looking at whether or not that exact specific question was a difficult question or not). However, I do not believe that adding such a granular piece of information, given that the question-level data are very random, would be a sound modeling choice. Therefore, I acknowledge that I had left out some additional explanatory power, but I believe the parsimonious model that I have constructed is the most intuitive one I can find and also getting a good predicted result.
...
The wording of this question is not entirely clear as to who are we predicting. Because if one person never makes the championships, it would be quite weird to claim that the person would get any significant amount (say >1) questions right in the championships at all, because the distributions between the two finalists and non-finalists are so different in the regular season. Therefore, I will interpret the question as follows. The idea of this questions goes something like this: The finalist roster is out, and a finalists A asks us how well will she do answering a championship question X. 
...
There are few pieces of information that we must consider before going into this question:
  ...
  + individuals are eliminated after the first two rounds in the finals if they are not doing well
  + championships data do not provide any question categories
  + we are limited to only running regressions on the regular season data, but looking at the finals data for general information is allowed.
...
Now that we are only left with the most bare-bones data, I think the most intuitive thing to do here is to look at how finalists do in the hardest questions in the regular season. It should be that individuals who do well on the most difficult questions should do well in the finals as well, where the questions are inherently more difficult. 

Following our definition of difficulty, I believe that take the questions where most people in one's cohort get incorrect. To get a sense of the difficulty, I must first choose what would be the cutoff for a "difficult question". While this sounds quite arbitrary, I will do my best to avoid introducing biases into the analysis. In particular, I will use a methodology that is extremely similar to the one from the previous question, where I simply substitute the "subject category" into the "difficult category". I will select the regular season questions where the finalist group gets similar correct rates as that of the championship data, where we have seen the distribution in question 1. After checking how the difficult question is for the finalists, I will see how well does the individual herself answer the specific question. Once again, I have taken the two-pronged approach, where the main determinants of getting a question right depends on individual's ability while accounting for checking how difficult was the question among similar individuals. 

NOTE: while we could easily account for whether or not an individual has participated in previous finals, where we expect previous participation will most likely increase one's chances of getting a question right in the finals, I will not include this aspect to this following regression. 
...
q3_df_scored <- q3_df %>% 
  ...
  # mutate(question_score=case_when(question_category==1 ~ 9,
  #                                    question_category==2 ~ 8,
  #                                    question_category==3 ~ 7,
  #                                    question_category==4 ~ 6,
  #                                    question_category==5 ~ 5,
  #                                    question_category==6 ~ 4,
  #                                    question_category==7 ~ 3,
  #                                    question_category==8 ~ 2,
  #                                    question_category==9 ~ 1)) %>%
  # group_by(user_id) %>% 
  # mutate(score_for_question = ifelse(correct==0,0,question_score)) %>% 
  # mutate(score=sum(score_for_question)/n()) 

q3_df_scored_filtered <- q3_df_scored %>% 
  filter(question_category<4) %>% 
  group_by(user_id,question_category) %>% 
  mutate(id_mean_correct_in_cat = mean(correct)*100,
         mean_correct_of_q = mean_correct_of_q*100)
...
finals_lm <- glm(data=q3_df_scored_filtered, correct ~ (id_mean_correct_in_cat) + (mean_correct_of_q) ,family="binomial")
...
q3_test_filtered <- q3_test_scored %>% 
  filter(round<3) %>%
  group_by(id,question_category) %>% 
  mutate(id_mean_correct_in_cat = mean(correct)*100,
         mean_correct_of_q = mean_correct_of_q*100)

predicted_possibilities_q3 <- predict(finals_lm, q3_test_filtered, type="response")

verdicts_q3 <- rep(0, dim(q3_test_filtered)[1])
verdicts_q3[predicted_possibilities_q3 > .5] = 1

test_results_q3 <- as.data.frame(cbind(verdicts_q3,q3_test_filtered$correct))
...
regression_coefficient_percents_q3 <- (exp(coef(finals_lm))-1)*100

regression_coefficient_percents_q3
```
Therefore, we have arrived at a pretty decent result once we test our model! In particular, the parsimonious logit model was able to accurately predict 74% of the championship questions correctly. Again, since we have used a logit model, some additional details should be provided. In particular, I have again multiplied all percent into percent units to ease the interpretation on the logit coefficient. The raw output will be given in the code, but I will spare the reader from the boring model. Simply, the logit model tells us that for every percent increase in the mean ability of finalist A to answer a question in the finals of a specific difficulty bracket correct, the probability of finalist A getting another question X in the same difficulty bracket goes up by 4.24%. On the other hand, if all the finalists collectively are one percent better at answering the questions of a specific category, then this particular finalist A will have a 1.39% higher chance of getting this question right as well.

Therefore, we have constructed a logit model which correctly estimates 74% of the finals data correctly without actually requiring finals data prior to testing. i.e. if we didn't need to show the accuracy or "how good" is this "good model", then we would never need to touch the finals data. 

NOTE: Below is the distribution of the regular season questions' mean correct rate among finalists used to train the model.

```{r}
q3_df_scored_filtered %>% 
  group_by(user_id) %>% 
  ggplot(aes(x=mean_correct_of_q)) +
  geom_histogram()
```
